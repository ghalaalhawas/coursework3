...
def reset_products():
    # Remove existing data from the shirts table
    db.execute("DELETE FROM shirts")

    # Add new product data to the shirts table
    products = [
        {
            "samplename": "iPhone",
            "description": "iPhone 13. Super Retina XDR display; 6.1‑inch (diagonal) all‑screen OLED display; 2532‑by‑1170-pixel resolution at 460 ppi; HDR display",
            "image": "iphone.jpg",
            "price": 799,
            "onSale": 0,
            "onSalePrice": 0,
            "typeClothes": "Electronics",
            "kind": "Smartphone"
        },
        {
            "samplename": "Laptop",
            "description": "1.4 GHz quad-core 8th gen Intel core i5. Intel Iris Plus Graphics 645. 16 GB 2133MHz LPDDR3. 256GB SSD",
            "image": "laptop.jpg",
            "price": 1299,
            "onSale": 1,
            "onSalePrice": 1199,
        ...
        {
            "samplename": "iPad",
            "description": "Single camera on the back with 12MP and 64GB or 256GB internal storage options",
            "image": "ipad.jpg",
            "price": 499,
            "onSale": 0,
            "onSalePrice": 0,
        ...
        {
            "samplename": "Keyboard",
            "description": "PSR-F52. Color/Finish. Body, Color, Black. Size/Weight. Dimensions, Width, 920 mm (36-1/4”). Height, 73 mm (2-7/8”). Depth, 266 mm (10-1/2”)",
            "image": "keyboard.jpg",
            "price": 49,
            "onSale": 1,
            "onSalePrice": 39,
    ...
    # Insert new product data into the shirts table
    for product in products:
        db.execute("""
            INSERT INTO shirts (samplename, description, image, price, onSale, onSalePrice, typeClothes, kind)
            VALUES (:samplename, :description, :image, :price, :onSale, :onSalePrice, :typeClothes, :kind)
        """, **product)

    return "Product data reset and added successfully!"
...
def index():
    ...
    if 'user' in session:
        shoppingCart = db.execute("SELECT samplename, description, image, SUM(qty), SUM(subTotal), price, id FROM cart GROUP BY samplename")
        shopLen = len(shoppingCart)
...
def buy():
    ...
    if session:
        ...
        db.execute("INSERT INTO cart (id, qty, samplename, description, image, price, subTotal) VALUES (:id, :qty, :samplename, :image, :price, :subTotal)", id=id, qty=qty, samplename=samplename, image=image, price=price, subTotal=subTotal)
        shoppingCart = db.execute("SELECT samplename, description, image, SUM(qty), SUM(subTotal), price, id FROM cart GROUP BY samplename")
        shopLen = len(shoppingCart)
...
def checkout():
    ...
    for item in order:
        db.execute("INSERT INTO purchases (uid, id, samplename, description, image, quantity) VALUES(:uid, :id, :samplename, :description, :image, :quantity)", uid=session["uid"], id=item["id"], samplename=item["samplename"], description=item["description"], image=item["image"], quantity=item["qty"])
    # Clear shopping cart
